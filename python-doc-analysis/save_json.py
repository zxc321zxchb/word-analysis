"""Test script to parse document and save to JSON file."""
import sys
import os
import json
...
def parse_to_json(sections):
    ...
    for index, section in enumerate(sections):
        ...
        if section.paragraphs or section.tables or section.images:
            section_data["content"] = {
                "text": section.paragraphs if section.paragraphs else [],
                "tables": [],
                "images": []
            }
        else:
...
def parse_and_save_json():
    """Parse document and save to JSON file."""
    print("Parsing real_test.docx and saving to JSON file...")
    ...
    json_result = parse_to_json(sorted_sections)
    ...
    print(f"Total sections: {json_result['document_info']['total_sections']}")
    
    print("\n" + "="*60)
    print("Preview of JSON content (first 3 sections):")
    print("="*60)
    preview = json_result.copy()
    preview["sections"] = json_result["sections"][:3]
