"""Test script to iteratively parse document from first paragraph number."""
import sys
import os
...
def iterative_parse_sections(sections):
    """Iteratively parse sections starting from first paragraph number.
...
def test_iterative_parsing():
    """Test iterative parsing of document starting from first paragraph number."""
    print("Testing iterative parsing of real_test.docx file...")
    ...
    if parsed.sections:
        print("\nStarting iterative parsing from first section...")
        # Sort sections by number_path to ensure correct order
        sorted_sections = sorted(parsed.sections, key=lambda s: s.number_path)
        # Limit to first 10 sections to avoid too much output
        iterative_parse_sections(sorted_sections[:10])
    else:
