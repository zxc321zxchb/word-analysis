"""Test script to parse document in hierarchical order."""
import sys
import os
...
def hierarchical_sort_key(section):
    ...
    # Convert number_path like "1.10.2" to a tuple of integers (1, 10, 2)
...
def iterative_parse_sections(sections):
    """Iteratively parse sections in hierarchical order.
...
def test_hierarchical_parsing():
    """Test parsing document in hierarchical order."""
    print("Testing hierarchical parsing of real_test.docx file...")
    ...
    if parsed.sections:
        print("\nStarting hierarchical parsing from first section...")
        # Sort sections in hierarchical order
        sorted_sections = sorted(parsed.sections, key=hierarchical_sort_key)
        # Limit to first 20 sections to avoid too much output
